...
const RadarCanvas: React.FC<RadarCanvasProps> = ({ scores }) => {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      drawRadar(ctx, size, scores, points, goodColorRef.current, badColorRef.current);
    };
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      drawRadar(ctx, sizeRef.current, scores, current, goodColorRef.current, badColorRef.current);
      if (progress < 1) {
